...
class DefectModel:
    ...
    def __init__(self):
        ...
        # self.model_path = os.path.join(self.model_dir, "defect.model")
    ...
    def generator(self, noise_imgs, train=True):
        ...
        with tf.variable_scope('generator'):
            ...
            # 对输入噪音图片进行线性变换
            # 64*8*3*3=4608
            z, h0_w, h0_b = self.linear(noise_imgs, # 图片数据
                                        self.gf_size * 8 * s_h16 * s_w16) # 输出值个数
            # reshape为合适的输入层格式
    ...
    def train(self):
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            ...
            # 训练之前，加载模型，从而实现增量训练 wdb add 20200403
            if os.path.exists("./model/checkpoint"):
                print("加载模型:", os.getcwd())
